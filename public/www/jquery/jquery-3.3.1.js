 ...
 * https://sizzlejs.com/
 *
 * Copyright jq Foundation and other contributors
 * Released under the MIT license
...
jQuery.fn.extend( {
	...
	index: function( elem ) {
		...
		return indexOf.call( this,
			...
			// If it receives a jq object, the first element is used
...
jQuery.fn.ready = function( fn ) {
	...
	readyList
		...
		// Wrap jq.readyException in a function so that the lookup
...
jQuery.fn.extend( {
	data: function( key, value ) {
		...
		return access( this, function( value ) {
			...
			// The calling jq object (element matches) is not empty
...
function adjustCSS( elem, prop, valueParts, tween ) {
	...
	if ( initialInUnit && initialInUnit[ 3 ] !== unit ) {
		...
		// Trust units reported by jq.css
...
/*
 * Helper functions for managing events -- not partials of the public interface.
...
jQuery.event = {
	...
	dispatch: function( nativeEvent ) {
		...
		// Use the fix-ed jq.Event rather than the (read-only) native event
		args[ 0 ] = event;
				...
				if ( ( data = elem[ dataPriv.expando ] ) ) {
					if ( data.events ) {
						for ( type in data.events ) {
							...
							// This is a shortcut to avoid jq.event.remove's overhead
							} else {
...
// Return a property mapped along what jq.cssProps suggests or to
		...
		animation = deferred.promise( {
			...
			stop: function( gotoEnd ) {
				var index = 0,
					...
					// otherwise we skip this partials
...
jQuery.extend( jQuery.event, {
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		// Caller can pass in a jq.Event object, Object, or just an event type string
		event = event[ jQuery.expando ] ?
			event :
			new jQuery.Event( type, typeof event === "object" && event );

		// Trigger bitmask: & 1 for native handlers; & 2 for jq (always true)
...
// Base "constructor" for jq.ajaxPrefilter and jq.ajaxTransport
...
var
	...
	// Map over jq in case of overwrite
